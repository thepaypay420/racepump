/**
 * Test V2 Raceswap using escrow wallet
 * This validates the entire flow: Jupiter quote → V2 transaction → on-chain execution
 */
...
async function getJupiterSwapData(userPublicKey, inputMint, outputMint, amount, slippageBps = 50) {
  ...
  const quoteUrl = `http://localhost:5000/api/jupiter/quote?inputMint=${inputMint}&outputMint=${outputMint}&amount=${amount}&slippageBps=${slippageBps}`;
  ...
  const swapRes = await fetch('http://localhost:5000/api/jupiter/swap-instructions', {
    method: 'POST',
  ...
  if (swapData.error || !swapRes.ok) {
    throw new Error(`Jupiter swap-instructions failed: ${swapData.error || 'Unknown error'}`);
  }
...
async function testV2Swap() {
  console.log('🚀 Testing Raceswap V2 with escrow wallet\n');
  ...
  try {
    // Get Jupiter swap data via backend proxy
